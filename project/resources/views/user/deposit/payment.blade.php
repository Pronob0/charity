            ...
            <div class="card shadow-md mb-4 ">
                ...
                <div class="card-body">
                    ...
                    <div class="row text-center">
                        <strong>@langg('Total Payment') : {{$currency->symbol.$deposit_data['amount']}}</strong>
                        @if ($charge)
                         <strong>@langg('Total Charge') : {{$currency->symbol.numFormat($charge,2)}}</strong>
                        @endif
                        <strong>@langg('Payment Method') : {{$gateway->name}}</strong>
                        <input type="hidden" name="currency" value="{{$currency->id}}">
                        @include('other.payment_load')

                        <div class="text-center my-4">
                            <button type="submit" id="payment__button" class="btn btn-primary">@langg('Submit')</button>
                        </div>
                    </div>
                </form>
                </div>
