...
@section('title')
   @langg('Manage Job Category')
@endsection

@section('breadcrumb')
 <section class="section">
    <div class="section-header justify-content-between">
        <h1> @langg('Manage Job Category')</h1>
        @if(access('add job category'))
            <a href="javascript:void(0)" data-toggle="modal" data-target="#addModal" class="btn btn-primary add"><i class="fas fa-plus"></i> @langg('Add New Job Category')</a>
        @endif
    </div>
</section>
@endsection
    ...
    <div class="col-12 col-md-12 col-lg-12">
        <div class="card">
                    ...
                    <div class="row flex-grow-1 flex-sm-grow-0">
                        <div class="col-sm-6 my-2">
                            <select class="form-control" id="" onChange="window.location.href=this.value">
                                ...
                                <option value="{{url('admin/job-category/list'.'?status=active')}}" {{request('status') == 'active'?'selected':''}}>@langg('Active')</option>
                                <option value="{{url('admin/job-category/list'.'?status=banned')}}" {{request('status') == 'banned'?'selected':''}}>@langg('Banned')</option>
                            </select>
                        </div>
                        <div class="col-sm-6 my-2">
            ...
            <div class="card-body text-center">
                <div class="table-responsive">
                    <table class="table table-striped">
                        ...
                        @forelse ($job_categories as $key => $value)
                          
                            <tr>
                                <td data-label="@langg('Sl')">{{$key + $job_categories->firstItem()}}</td>
                                ...
                                <td data-label="@langg('Action')">
                                    <a class="btn btn-primary details" data-category="{{$value}}" href="javascript:void(0)" data-route="{{route('admin.jcategory.update',$value->id)}}">@langg('Edit')</a>

                                    <a class="btn btn-danger delete" data-staff="{{$value}}" data-toggle="modal" data-target="#exampleModal" href="javascript:void(0)" data-route="{{route('admin.jcategory.delete',$value->id)}}">@langg('Delete')</a>
                                </td>
            ...
            @if ($job_categories->hasPages())
                {{ $job_categories->links('admin.partials.paginate') }}
            @endif
        </div>
    </div>
</div>
...
<div class="modal fade" id="addModal" tabindex="-1" role="dialog" aria-labelledby="modelTitleId" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <form action="{{route('admin.jcategory.store')}}" method="POST">
            @csrf
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">@langg('Add New Job Category')</h5>
...
@push('script')
    <script>
        ...
        $('.details').on('click',function () { 
            $('#addModal').find('.modal-title').text("@langg('Edit Job Category')")
